                        ...
                        <form action="{{ route('ListingsFilter') }}" class="header-search-form" method="get" onsubmit="return validateSearch()">
                            <div class="header-search-wrap d-flex align-items-center">
                                ...
                                <select class="search-select" name="type" id="type">
                                    ...
                                    <option value="hotel" @if ($currect_route_name == 'hotel.home' || request()->type == 'hotel') selected @endif>{{ get_phrase('Hotel') }}</option>
                                    <option value="restaurant" @if ($currect_route_name == 'restaurant.home' || request()->type == 'restaurant') selected @endif>{{ get_phrase('Restaurant') }}</option>
                                    <option value="real-estate" @if ($currect_route_name == 'real-estate.home' || request()->type == 'real-estate') selected @endif>{{ get_phrase('Real-Estate') }}</option>
                                    <option value="car" @if ($currect_route_name == 'car.home' || request()->type == 'car') selected @endif>{{ get_phrase('Car') }}</option> --}}
                                     @php 
                                        $types = App\Models\CustomType::where('status', 1)->orderBy('created_at', 'desc')->get();  
                                    @endphp 
