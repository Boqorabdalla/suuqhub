...
<header class="header-section">
                            ...
                            <div class="header-search-wrap d-flex align-items-center">
                                ...
                                <select class="search-select" name="type" id="type">
                                    ...
                                    @foreach($types as $type)
                                       <option value="{{$type->slug}}" @if (request()->type == $type->slug) selected @endif>{{ $type->name }}</option>
                                    @endforeach
                                </select>
                                        ...
                                        <ul class="header-nav-list">
                                              @php
                                                ...
                                                @foreach ($menu_items as $index => $item)
                                                    ...
                                                    @if ($index < 2)
                                                        <li>
                                                            <a class="first-a {{ $isActive ? 'active' : '' }}" href="{{ $url }}">
                                            ...
                                            <li><a href="{{ route('shop') }}" class="first-a {{ request()->routeIs('shop') || request()->routeIs('shop.product') ? 'active' : '' }}">{{ get_phrase('Shop') }}</a></li>
                                            <li><a href="{{ route('shop.track') }}" class="first-a {{ request()->routeIs('shop.track') ? 'active' : '' }}">{{ get_phrase('Track Order') }}</a></li>
                                            @endif
    ...
    @csrf
    <button type="submit" class="btn bg-transparent border-0 text-start w-100 ...">
        Log Out
    </button>
</form>
                                ...
                                @else
                                    <a href="{{ route('login') }}" class="login">{{ get_phrase('Login') }}</a>
                                @endif
                                @if (addon_status('shop') == 1 && auth()->check())
                                    <a href="{{ route('shop.track') }}" class="btn btn-icon btn-light ms-2" title="{{ get_phrase('Track Order') }}">
                                        <i class="bi bi-truck"></i>
                                    </a>
                                    <a href="{{ route('shop.wishlist') }}" class="btn btn-icon btn-light ms-2" title="{{ get_phrase('Wishlist') }}">
                                        <i class="bi bi-heart"></i>
                                    </a>
                                    <a href="{{ route('shop.cart') }}" class="btn btn-icon btn-light position-relative ms-2" title="{{ get_phrase('Cart') }}">
                                        <i class="bi bi-cart"></i>
                                        @php
                                            $cartCount = \App\Models\ShopCartItem::where('user_id', auth()->id())->sum('quantity');
                                        @endphp
                                        @if($cartCount > 0)
                                    ...
                                    <!-- Notifications Bell -->
                                    @php
                                        $unreadNotifications = \App\Models\ShopNotification::where('user_id', auth()->id())
                                            ->whereNull('read_at')
                                            ->orderBy('created_at', 'desc')
                                            ->take(5)
                                            ->get();
                                        $unreadCount = \App\Models\ShopNotification::where('user_id', auth()->id())
                                            ->whereNull('read_at')
                                            ->count();
                                    @endphp
                                    <div class="dropdown">
                                        <a href="#" class="btn btn-icon btn-light ms-2 position-relative" data-bs-toggle="dropdown" title="{{ get_phrase('Notifications') }}">
                                            <i class="bi bi-bell"></i>
                                            @if($unreadCount > 0)
                                                <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger notification-badge" style="font-size: 10px;">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
                                            @endif
                                        </a>
                                        <div class="dropdown-menu dropdown-menu-end notification-dropdown" style="width: 320px; max-height: 400px; overflow-y: auto;">
                                            <div class="notification-header d-flex justify-content-between align-items-center px-3 py-2 border-bottom">
                                                <h6 class="mb-0">{{ get_phrase('Notifications') }}</h6>
                                                @if($unreadCount > 0)
                                                    <a href="{{ route('shop.notifications') }}" class="text-small">{{ get_phrase('View All') }}</a>
                                                @endif
                                            </div>
                                            @if($unreadNotifications->count() > 0)
                                                @foreach($unreadNotifications as $notification)
                                                    @php $notifData = is_string($notification->data) ? json_decode($notification->data, true) : $notification->data; @endphp
                                                    <a href="{{ route('shop.order', $notifData['order_id'] ?? 0) }}" class="notification-item px-3 py-2 border-bottom {{ $notification->read_at ? 'read' : 'unread' }}">
                                                        <div class="d-flex align-items-start gap-2">
                                                            ...
                                                            <div class="flex-grow-1">
                                                                <p class="mb-0 text-small">{{ $notifData['message'] ?? $notifData['title'] }}</p>
                                            ...
                                            @else
                                                <div class="px-3 py-4 text-center text-muted">
                                                    ...
                                                    <p class="mb-0">{{ get_phrase('No notifications') }}</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
