    ...
    <div class="ol-card-body p-3">
        <form action="{{route('admin.create.user',['type'=>$type])}}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                @if(auth()->user()->role == 1)
                <div class="col-sm-6 mb-3">
                    <label for="role" class="form-label ol-form-label"> {{get_phrase('User Role *')}} </label>
                    <select name="role" id="role" class="form-control ol-select2 select2-hidden-accessible ol-form-control" required>
                        <option value="2"> {{get_phrase('Agent')}} </option>
                        <option value="3"> {{get_phrase('Order Manager')}} </option>
                    </select>
                    <small class="text-muted">{{ get_phrase('Order Manager can only manage shop orders and deliveries') }}</small>
                </div>
                @endif
                <div class="col-sm-6 mb-3" id="directory_type_div" style="display:none;">
                    <label for="directory_type" class="form-label ol-form-label"> {{get_phrase('Directory Type *')}} </label>
                    <select name="directory_type" id="directory_type" class="form-control ol-select2 ol-form-control">
                        <option value=""> {{get_phrase('Select Directory')}} </option>
                        @foreach(App\Models\CustomType::where('status', 1)->get() as $type)
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    var dirDiv = document.getElementById('directory_type_div');
