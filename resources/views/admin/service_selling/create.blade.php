...
<form id="form-action" action="{{route('admin.service.stores')}}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control ol-form-control" name="video" id="video" placeholder="{{ get_phrase('Paste a YouTube video link here') }}" required>
    </div>
      <div class="mb-3">
            <label for="status" class="form-label ol-form-label cap-form-label"> {{get_phrase('Status')}} </label>
            <select name="status" id="status" class="form-control ol-form-control ol-select2" required>
                ...
                <div id="slot-container">
                    <div class="row mb-2 slot-row">
                        <div class="col-md-4">
                            ...
                            <select name="slots[0][day]" class="form-control ol-form-control " required>
                        ...
                        <div class="col-md-2 ">
                            <div class="d-flex gap-1" style="margin-top: 30px;">
...
<script>
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('add-slot')) {
            ...
            newRow.innerHTML = `
                <div class="col-md-4">
                    <select name="slots[${slotIndex}][day]" class="form-control ol-form-control" required>
                        <option value="">{{ get_phrase('Select Day') }}</option>
                        <option value="Sunday">{{ get_phrase('Sunday') }}</option>
                        <option value="Monday">{{ get_phrase('Monday') }}</option>
                        <option value="Tuesday">{{ get_phrase('Tuesday') }}</option>
