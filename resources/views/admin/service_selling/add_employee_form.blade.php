<form action="{{route('admin.employee.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="email" class="form-control ol-form-control" name="email" id="email" placeholder="{{get_phrase('Enter Email')}}" required>
    </div>
    <div class="mb-3">
        <label for="phone" class="form-label ol-form-label"> {{get_phrase('Phone')}} </label>
        <input type="number" class="form-control ol-form-control" name="phone" id="phone" placeholder="{{get_phrase('Enter Phone')}}" required>
    </div>
